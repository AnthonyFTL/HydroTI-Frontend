...
const Irrigations = ({dispatch}) => {
    ...
    const onPowerClick = () => DataService.switchPumpValue()
        .then((response) => {
        return response;
    });
    ...
    // setTimeout(function () {
    //     if (iotData != null && iotData.moisture != null && iotData.lights != null) {
    //         if (iotData.moisture <= 15 && iotData.lights <= 25) {
    //             IrrigationService.setAutomaticIrrigationValue(true).then(() => {});
    //             console.log('automatically starting irrigation')
    //         } else {
    //             IrrigationService.setAutomaticIrrigationValue(false).then(() => {});
    //             console.log('automatically stopping irrigation')
    //         }
    //     }
    // }, 5000);

    //
    // if (iotData.moisture <= 15 && iotData.lights){
    // }
        ...
        iotData && (
            <>
                ...
                <Confirmation
                    ...
                    onConfirm={() => {
                        onPowerClick().then((response) => {
                            setToggleOnOffConfirmationIsOpen(false);
                            console.log(response);
                            const startDate = response.data.irrigationEntity.startDate;
                            const endDate = response.data.irrigationEntity.endDate;
                            const irrigationDuration = response.data.irrigationEntity.irrigationDuration;
                            console.log(startDate);
                            console.log(endDate);
                            console.log(irrigationDuration);
                        });
                    }}
                />
            </>
